...
RSpec.describe Session::UserSession do
  let(:user) { create(:user) }

  subject    { Session::UserSession.new(user: user, password: user.password) }
  ...
  describe '#save' do
    ...
    context do
      ...
      before { expect(Token::UserToken).to receive(:encode).with(user_id: user.id).and_return('x-y-z') }
